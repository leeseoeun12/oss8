...
- 2) 공동 작업(1)
  ...
  *주의*: 사용 중인 버전보다 최신인 파일에서 참조하려면 최신 업데이트가 설치되어 있어야 한다.
  ...
  - *부품 파일*(더 최신인 부품파일을 이용하려면) : 파일 - 열기 선택 or 부품 파일에서 관리 탭 - 삽입 팬러 - 파생 클릭 -> 배치하려는 최신 버전 부품 파일(.ipt)을 찾아 선택. (마지막으로 저장한 inventor 버전이 표시됨.) -> 필요한 경우 옵션을 클릭하여 파일 옵션 대화 상자를 열어 설계 뷰 표현 지정. -> 구성요소 배치 대화상자에서 열기 클릭 시. -> 솔리드 곡면 본체 작성.
