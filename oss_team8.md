...
- 2) 공동 작업(1)
   **a. 공유 뷰를 통한 공동 작업**
     : 공유 뷰를 이용하여 온라인으로 시각적 표현에 대한 공동 작업을 수행할 수 있다. 사용자가 제공하는 링크를 이용하여 Autodesk가 설치되어 있지 않더라도 누구든지 공유 뷰를 보고 주석을 추가할 수 있다. 다른 사용자가 공유 뷰에 대한 주석 추가 시 사용자에게 전자 메일이 전송된다.
     - *공유 뷰 정보* : 공유 뷰 작성 시 팝업 창에서 링크가 제공되고, 이를 공동 작업자에게 쉽게 보낼 수 있음. 공동 작업자는 Autodesk 제품이 없어도 되며, Autodesk 뷰어에서 직접 주석 게시 가능.
     - *공유 뷰 리스트 정보* : 사용자는 공유 뷰 리스트를 이용해 빠르게 공유 뷰에 접근하고, 새 주석 스레드를 시작하고, Autodesk 제품에서 주석에 응답할 수 있음. 만료일 연장, 공유 뷰 삭제 등의 관리 옵션 또한 제공.
    
   **b. 데이터 이용 작업**
     : 다른 CAD 응용프로그램에서 작성하고 저장한 3D 모델을 열 때 연관 링크를 유지할 수 있다. 
     - *AnyCAD for Inventor* : 사용 중인 버전보다 1년 더 최신인 부품 또는 조립품 파일을 참조할 수 있음. 파일은 최신 버전의 Inventor 파일에 연관됨. 열기, 배치, 파생 명령을 사용하여 최신 버전 파일에 대한 참조를 작성.
